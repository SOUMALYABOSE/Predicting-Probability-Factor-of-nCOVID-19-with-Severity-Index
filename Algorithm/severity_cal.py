...
data=pd.read_csv("C:\\Users\\Ankush\\Desktop\\ML Assignment\\arko\\data.csv",delimiter=',',index_col=0).fillna('not')
...
def f1(x):
    return (((95-x)*20)/3)+90
def f2(x):
    return (((95-x)*20)/3)+200
x = np.arange(40,100,1)
y1 = f1(x)
y2 = f2(x)
plt.plot(x,y1,'k--')
plt.plot(x,y2,'k--')
plt.fill_between(x, y1, y2, color='#FC8CCE')
